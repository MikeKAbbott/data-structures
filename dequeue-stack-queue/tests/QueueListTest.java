...
public class QueueListTest {
    QueueList empty = new QueueList<>();
    QueueList one = new QueueList<>();
    QueueList three = new QueueList<>();
    QueueList many = new QueueList<>();
    ...
    public void remove() {
        try {
            ...
            three.offer(3);
            many.offer(4);
            assertEquals(1, empty.remove());
            assertEquals(1, one.remove());
            assertEquals(0, three.remove());
            assertEquals(0, many.remove());
        }catch (NoSuchElementE E){}
    }
